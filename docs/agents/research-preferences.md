...
- **Prose voice — rewrite, don't just segment** (three-way verdict, 2026-07-19): the owner's preferred register is the spartan style — concrete-first section openers, short direct sentences, hedgy connectives and template transitions cut; paragraphs of 2–4 sentences that carry a whole argument. Mechanical segmentation (one-sentence fragments) overshoots: it reads staccato and severs claims from their qualifiers, which costs synthesis, not just style. Bullets only where content is genuinely enumerable.
