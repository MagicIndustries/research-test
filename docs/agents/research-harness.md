...
- **Derived stacks** (owner-endorsed, 2026-07-18): the strongest way to hold the research stage constant is to not re-run it — a stack variant may reuse a completed run's output as its intermediate, named with a `--from-<run>` suffix (e.g. `daymade-deep-research+pocock-edit-article--fable--cc--from-r2`), the reuse recorded as a RUN.md deviation and the copy diff-verified. Different writing stages applied to the same intermediate are then exactly comparable.
- **Edit-stage dispatch rules** (from the first writing-stage run and its fidelity check): (1) article-oriented edit skills edit the article-like files only — resources lists and evidence trails (`research-notes/` and kin) pass through byte-identical; (2) **never add a citation marker to text that lacked one** — sentence-splitting tempts the editor to re-stamp citations onto fragments and silently extend attribution to editorial text ("citation re-stamping", 3/8 drift in the first fidelity check); (3) formatting targets the **rendered** output (see the judge briefing above) — reference lists become bullet lists.
