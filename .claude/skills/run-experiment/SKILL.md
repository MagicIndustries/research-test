...
2. **Derive the variant name** from the grammar (skill stack, model, harness, settings suffix only when non-default). If `runs/<variant>/` already exists, stop and ask — re-runs are deliberately undecided. Done when: the variant directory name is fixed and unoccupied.
...
5. **Execute the stack** against the canonical prompt exactly as written — no edits, no additions. Direct output into `runs/<variant>/output/`; if the stack wrote elsewhere, move the files unmodified. Done when: `output/` holds the run's complete output, byte-identical to what the stack produced.
6. **Finalize**: update RUN.md (`status: complete`, duration/tokens/cost as capturable, deviations noted), add the variant's row to PROMPT.md's matrix, commit, push, open a PR titled `run(<experiment>): <variant>`. Done when: the PR is open and its diff contains only this run's files plus the matrix row.
