...
4. **Finalize**: matrix row in PROMPT.md, a dated entry appended to the experiment's `OBSERVATIONS.md` (anything noticed the structured files don't capture), commit, push, PR titled `run(<experiment>): <variant> (retrofit)`. Done when: the PR is open and its diff contains only this run's files, the matrix row, and the observations entry.
