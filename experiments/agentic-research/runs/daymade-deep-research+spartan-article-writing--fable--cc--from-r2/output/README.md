...
What makes LLM agents produce excellent research reports — the methods, skills, prompts, and tools, mapped as four approaches. Produced unattended by the `daymade-deep-research` skill (V6.1, Standard mode). Start with the summary; drill into an approach when you need it.

## Contents

| File | What it is |
|---|---|
| **[summary.md](summary.md)** | Main report, TL;DR first: key terms, history, the four-approach map, shared methodological ingredients, evaluation literature, gaps, controversies, and recommended steps for a solo Claude Code developer with a skill-comparison harness |
...
- **Deciding what to build on:** the §3 table, then the two "assemble it yourself" subdirectories.
- **Building the judging side of a harness:** summary §5, plus the [multi-agent-pipelines/resources.md](multi-agent-pipelines/resources.md) entry for [7] and the DeepResearch Bench entries in the registry.
