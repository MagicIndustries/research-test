...
Self-hosted frameworks that reproduce the deep-research loop with your own model and search keys. Permissively licensed across the board, architecturally diverse — from 500-line recursive loops to full LangGraph supervisor pipelines — with one quantified quality gap versus hosted products.
...
## Eight repos at a glance (observed 2026-07-18)
...
**GPT Researcher** — the most actively maintained "classic" deep-research agent. A planner agent generates research questions, parallel crawler/execution agents gather per-question evidence, and a publisher aggregates the report. Model-provider-agnostic, installs via pip/Docker/Colab, and is now also installable as a Claude Skill [16][28].

**LangChain Open Deep Research** — the reference orchestrated pipeline (see `../multi-agent-pipelines/README.md` for the architecture in depth): supervisor delegation into parallel context-isolated subagents, each ending with a citation-writing clean-up call [17][25]. LangChain self-reported #6 / 0.4344 on Deep Research Bench (2025-08) [25].

**STORM (Stanford)** — the academic entry (NAACL 2024). The pre-writing stage does perspective-guided question asking through a simulated conversation between a Wikipedia-writer agent and a topic expert, producing an outline plus references; the writing stage generates the cited article. Ships as pip-installable `knowledge-storm` with litellm model-agnosticism and pluggable retrievers (You.com, Bing, or VectorRM over your own documents) [24][29]. Effectively a completed research artifact — last push 2025-09-30 [19].

**Hugging Face open Deep Research** — not a standalone framework but an example inside smolagents: a CodeAgent-based replication of OpenAI Deep Research scoring **55% pass@1 on GAIA validation vs 67% for OpenAI's original** — the only quantified open-vs-hosted gap found [26][20].

**Minimalist loops** — dzhng/deep-research deliberately stays under ~500 lines of TypeScript: user-set breadth/depth parameters drive a recursive loop that generates SERP queries, extracts "learnings" and follow-up directions, and emits a Markdown report [21]. Jina's node-DeepResearch runs a single search-read-reason loop "until it finds the answer or exceeds the token budget" [22]. These are the best pedagogical reading for understanding the core loop.

**Tongyi DeepResearch (Alibaba)** — the odd one out: it centers on a purpose-trained open deep-research *model* rather than orchestrating third-party LLMs [23].

**DeerFlow (ByteDance)** — carries the biggest caveat. DeerFlow 2.0 (Feb 2026, #1 on GitHub Trending 2026-02-28) is a ground-up rewrite repositioned as a general "super agent harness" (subagents, sandboxes, memory, skills, IM channels, Claude Code integration); the original LangGraph deep-research framework survives only on the `main-1.x` branch [27][18]. Its 77K stars should **not** be credited to the deep-research category.

## Getting going

Common pattern across all frameworks: supply at least one LLM API key **plus** a web-search provider key (SerpAPI/Serper/Tavily/etc.). That's the main friction versus hosted products, where search and model come bundled [28][25][26][29].
