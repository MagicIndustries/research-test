...
> One well-engineered instruction package turns a capable general agent (Claude Code, claude.ai, or any Agent-Skills-compatible host) into a research agent.
>
> No orchestration infrastructure — the "system" is a folder of markdown and scripts.
...
Anthropic launched **Agent Skills** on October 16, 2025 as "folders that include instructions, scripts, and resources that Claude can load when needed," working across the Claude apps, Claude Code, and the API [30].

A skill is a directory whose required `SKILL.md` carries YAML frontmatter (`name`, `description`) plus procedural instructions; it may bundle arbitrary additional files [31].
...
Because loading is filesystem-mediated, bundled context is "effectively unbounded" for agents with a filesystem and code execution [31].

Deep-research skills exploit exactly that: multi-phase pipelines, personas, citation validators, and report templates all ride in one folder without polluting the context window [31].

In Claude Code specifically, skills load from personal (`~/.claude/skills/`), project (`.claude/skills/`), plugin, or enterprise scopes, and are invocable as `/skill-name` [48].

They follow the cross-tool Agent Skills open standard (agentskills.io) and can be forced to execute inside a subagent [48].
...
Notably, these ingredients — effort scaling, parallel retrieval, subagent fan-out, citation verification, external memory — are the same ones Anthropic's orchestrated multi-agent system describes [7].

One skill folder converged on them independently [36]. The two "assemble it yourself" approaches are converging in practice.
